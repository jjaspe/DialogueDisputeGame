...
namespace DialogueDisputeGameClient.WCF
{
    ...
    {
        ChannelFactory<IDisputeGameConnectionService> factory;
        IDisputeGameConnectionService remoteProxy;
        DataPlayer player;
        ...
        List<IConnectionObserver> myObservers;
        ...
        {
            myObservers = new List<IConnectionObserver>();
        }
        ...
        public void parseRequest(Messages.GameMessages header, List<object> args, object sender)
        {
            ...
            switch (header)
            {
                ...
                case Messages.GameMessages.somChosen:
                    remoteProxy.addDataMessage(player, Messages.GameMessages.somChosen, args[0].ToString());
                    break;
                #endregion
                default:
                    break;
        ...
        {
            remoteProxy.addDataMessage(player, Messages.GameMessages.JoinMatch, name);
        }
        ...
        {
            factory = new ChannelFactory<IDisputeGameConnectionService>("ClientConfig");
            remoteProxy = factory.CreateChannel();
            DataPlayer player= remoteProxy.connect(playerName);
            if (player != null)
            {
                this.player = player;
        ...
        {
            getNewMessages();
            foreach (WCFMessage msg in serverMessages)//Get every message
            {
                parseNewMessage(msg);
                foreach (IConnectionObserver obs in myObservers)//Send to every observer
                {
